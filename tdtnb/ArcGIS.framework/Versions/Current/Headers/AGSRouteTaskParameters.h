...
@interface AGSRouteTaskParameters : NSObject <AGSCoding> {
 @private
	...
	NSString *_directionsTimeAttributeName;
	BOOL _doNotLocateOnRestrictedElements;
	BOOL _findBestSequence;
	BOOL _ignoreInvalidLocations;
	NSString *_impedanceAttributeName;
	double _outputGeometryPrecision;
	AGSUnits _outputGeometryPrecisionUnits;
	AGSNAOutputLine _outputLines;
	AGSSpatialReference *_outSpatialReference;
	id _polygonBarriers; 
	id _polylineBarriers; 
	BOOL _preserveFirstStop;
	BOOL _preserveLastStop;
	NSArray *_restrictionAttributeNames;
	AGSNAUTurn _uTurns; 
	BOOL _returnPointBarriers;
	BOOL _returnDirections;
	BOOL _returnPolygonBarriers;
	BOOL _returnPolylineBarriers;
	BOOL _returnRouteGraphics;
	BOOL _returnStopGraphics;
	NSDate *_startTime;
	id _stops;
	BOOL _useHierarchy;
